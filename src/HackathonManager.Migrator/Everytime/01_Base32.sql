...
begin
    if length(s) <> 26 then
        raise exception 'base32 encoded uuid must be 26 characters';
    end if;
    ...
    then
        raise exception 'base32 encoded uuid must only contain characters in the base32 alphabet';
    end if;

    if chr(get_byte(v, 0)) > '7' then
        raise exception 'base32 encoded uuid must start with 0-7';
    end if;
